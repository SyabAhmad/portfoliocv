...
const CertificationBadges = ({ certifications, maxItems = 12, showCategory = true }) => {
 ...
 {/* Date Badge */}
 <div className="absolute top-1 right-1 bg-slate-900/80 backdrop-blur-sm px-1.5 py-0.5 rounded text-xs text-stone-300 border border-slate-700/50">
 ...
 <div>
 <p className="text-stone-400 text-xs font-semibold mb-2">
 SKILLS DEMONSTRATED
 </p>
 ...
 <span
 key={idx}
 className="px-2 py-1 bg-slate-800 border border-slate-600 rounded text-xs text-stone-300"
 >
 {skill.trim()}
 </span>
 ))}
 ...
 <p className="text-xs text-stone-500 mb-1">Credential ID</p>
 <p className="text-sm text-stone-300 font-mono break-all">
 {selectedBadge.credentialId}
 </p>
 </div>
 )}
